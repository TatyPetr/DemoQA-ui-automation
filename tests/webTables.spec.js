...
test.describe('Elements - Web Tables', () => {
  ...
  test('TC-004: should search record by existing first name', async ({ webTablesPage }) => {
    await webTablesPage.searchUser(webTableData.searchText);
    const foundNameCell = webTablesPage.getCellByText(
      webTableData.searchText
    );
    await expect(foundNameCell).toBeVisible();
    await expect(foundNameCell).toHaveText(webTableData.searchText);
  });
 
  test('TC-005: should edit an existing record in Web Tables', async ({ webTablesPage }) => {
    const user = {
      ...webTableData,
    ...
    await expect(
      webTablesPage.getCellByText(user.email)
    ).toBeVisible();
 
    await expect(
      webTablesPage.getCellByText(updatedDepartment)
    ).toBeVisible();
  });
 
  test('TC-006: should delete a created record from Web Tables', async ({ webTablesPage }) => {
    const user = {
      ...webTableData,
